...
public class Elevator implements Subsystem
{
	...
	{
		return (level.getLevel() > previousLevel.getLevel()) ? m_motor.getPosition().abs(Meters) >= level.getLevel()
				: m_motor.getPosition().abs(Meters) <= level.getLevel();
	}
	...
	public Command getLevelCommand(ElevatorState level)
	{
		...
		return new Command()
		{
			...
			{
				return passedLevel(level, m_level);
			}
