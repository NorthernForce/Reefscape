...
public class Dashboard extends SubsystemBase
{
    ...
    public Pose2d getTargetPoseLeft(Pose2d currentPose)
    {
        ...
        {
            if (pose.getDistanceFromLeft(currentPose).in(Meters) < pose.getDistanceFromLeft(closestTranslation).in(Meters))
            {
    ...
    public Pose2d getTargetPoseCenter(Pose2d currentPose)
    {
        ...
        for (ReefSide pose : FieldConstants.ReefPositions.SIDES)
        {
            if (pose.getDistanceFromCenter(currentPose).in(Meters) < pose.getDistanceFromCenter(closestTranslation).in(Meters))
            {
    ...
    public Pose2d getTargetPoseRight(Pose2d currentPose)
    {
        ...
        for (ReefSide pose : FieldConstants.ReefPositions.SIDES)
        {
            if (pose.getDistanceFromRight(currentPose).in(Meters) < pose.getDistanceFromRight(closestTranslation).in(Meters)
