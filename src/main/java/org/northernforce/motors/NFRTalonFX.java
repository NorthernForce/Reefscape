...
/**
 * This is a class designed for interfacing with a group of TalonFX's. This uses
 * Phoenix 6 API.
 */
public class NFRTalonFX extends TalonFX implements NFRMotorController
{
	/**
	 * This class is the hardware interface for the integrated encoder. Uses Phoenix
	 * 6 API.
	 */
	public class IntegratedEncoder implements NFREncoder
	{
		...
		/**
		 * Checks to see if encoder is present by checking to see whether motor is
		 * present.
		 */
		@Override
		...
		/**
		 * Gets the position of the integrated encoder. Factors in conversion factor and
		 * inversion.
		 */
		@Override
		...
		/**
		 * Resets the encoder position. Do not do this too often. Could interfere with
		 * odometry.
		 * 
		 * @param position the new position of the encoder. Implementation factors in
		 *                 conversionFactor
		 */
		@Override
		...
		/**
		 * Sets the position using the simulation API. Use solely in simulation as it
		 * can lead to undefined behavior on a robot.
		 * 
		...
		/**
		 * Adds to the position using the simulation API. Use solely in simulation as it
		 * can lead to undefined behavior on a robot.
		 * 
		 * @param position of the encoder which is affected by the conversion factor.
		 */
		...
		/**
		 * Sets the velocity using the simulation API. Use solely in simulation as it
		 * can lead to undefined behavior on a robot.
		 * 
		...
		/**
		 * Gets the conversion factor that affects readings of the sensor. This is by
		 * default 1.
		 * 
		 * @return the factor for measurements of velocity and position. 1 means 1 unit
		 *         = 1 encoder rotation.
		 */
		@Override
	...
	/**
	 * Checks to see whether an motor controller is present by comparing the version
	 * to -1.
	 * 
	 ...
	 * 
	 * @return a list of all of the motor controllers including the primary and the
	 *         followers
	 */
	@Override
	...
	/**
	 * Sets the selected encoder to a specific encoder that inherits from
	 * NFREncoder. This should be used only with encoders that are compatible with
	 * the motor controller.
	 * 
	 * @param encoder to link to the motor controller
	 * @throws MotorEncoderMismatchException an exception if the encoder is not
	 *                                       compatible to be directly linked to the
	 *                                       motor
	 */
	@Override
	...
	/**
	 * Fuses the cancoder to the rotor sensor, using a combination for more accurate
	 * and quicker results.
	 * 
	...
	/**
	 * Returns the selected encoder that is linked to the motor controller. By
	 * default, the integrated encoder is chosen.
	 * 
	...
	/**
	 * Returns the external quadrature encoder plugged into the motor. May not be
	 * able to tell if the external quadrature is present however.
	 * 
	...
	/**
	 * Returns the external absolute encoder plugged into the motor. May not be able
	 * to tell if the absolute is present however.
	 * 
	...
	/**
	 * Sets the velocity of the motor using closed-loop feedback. Must use a
	 * configured pid slot.
	 * 
	...
	{
		setControl(velocityDutyCycle.withVelocity(velocity / getSelectedEncoder().getConversionFactor())
				.withSlot(pidSlot));
	}
	 ...
	 * 
	 * @param pidSlot              the pid slot of the closed-loop configuration.
	 * @param velocity             the target velocity in terms of the selected
	 *                             sensor's units.
	 * @param arbitraryFeedforward the arbitrary percentage value applied. Often to
	 *                             counteract gravity. Added to the output
	 *                             calculated by the closed-loop control.
	 */
	@Override
	public void setVelocity(int pidSlot, double velocity, double arbitraryFeedforward)
	{
		setControl(velocityDutyCycle.withVelocity(velocity / getSelectedEncoder().getConversionFactor())
				.withSlot(pidSlot).withFeedForward(arbitraryFeedforward));
	}

	/**
	 * Sets the position of the motor using closed-loop feedback. Must use a
	 * configured pid slot.
	 * 
	 * @param pidSlot  the pid slot of the closed-loop configuration.
	...
	{
		setControl(positionDutyCycle.withPosition(position / getSelectedEncoder().getConversionFactor())
				.withSlot(pidSlot));
	}
	 ...
	 * 
	 * @param pidSlot              the pid slot of the closed-loop configuration.
	 * @param position             the target position in terms of the selected
	 *                             sensor's units.
	 * @param arbitraryFeedforward the arbitrary percentage value applied. Often to
	 *                             counteract gravity. Added to the output
	 *                             calculated by the closed-loop control.
	...
	/**
	 * Sets the position of the motor using trapezoidal closed-loop feedback. Must
	 * use a configured pid slot.
	 * 
	 * @param pidSlot  the pid slot of the closed-loop configuration.
	 * @param position the target position in terms of the selected sensor's units.
	 */
	...
	{
		setControl(motionMagicVoltage.withSlot(pidSlot)
				.withPosition(position / getSelectedEncoder().getConversionFactor()));
	}
	...
	/**
	 * Gets the simulation output voltage that is calculated by inputs to the motor.
	 * This should not be used when not in simulation, as it can lead to undefined
	 * behavior.
	 * 
	...
	/**
	 * Sets the opposition of a follower so it opposes the inversion of the primary
	 * motor.
	 * 
	...
	/**
	 * Sets up a limit that prevent the motor from moving in all modes when past
	 * this limit.
	 * 
	...
	/**
	 * Returns the target position that the closed-loop control is attemped to
	 * reach.
	 * 
