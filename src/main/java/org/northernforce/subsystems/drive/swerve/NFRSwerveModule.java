...
public class NFRSwerveModule extends NFRSubsystem
{
	...
	public static class NFRSwerveModuleConfiguration extends NFRSubsystemConfiguration
	{
		...
		/**
		 * Creates a new basic swerve module configuration. All doubles are initialized
		 * to zero.
		 * 
		 ...
		 * 
		 * @param name           the name of the swerve module (ex. Back Left)
		 * @param driveGearbox   the gearbox for the drive motor. Only needed for
		 *                       simulation.
		 * @param turnGearbox    the gearbox for the turn motor. Only needed for
		 *                       simulation.
		 * @param driveGearRatio the gear ratio for the drive shaft. Only used in
		 *                       simulation.
		 * @param turnGearRatio  the gear ratio for the turn shaft. Only used in
		 *                       simulation.
		 ...
		 * @param maxSpeed       the maximum speed of the drive. Used for closed-loop
		 *                       conversions from [-1, 1] to [-maxSpeed, maxSpeed].
		 */
	 ...
	 * @param externalEncoder the external encoder, if not linked directly to the
	 *                        turnController.
	 */
	 ...
	 * 
	 * @param velocity the speed of the drive motor in units relative to the
	 *                 selected encoder.
	...
	public void setTurnPosition(Rotation2d position, int pidSlot, boolean useTrapezoidalPositioning)
	{
		...
		{
			turnController.setPositionTrapezoidal(pidSlot, MathUtil.inputModulus(position.getRotations(), 0, 1));
		} else
		{
	...
	/**
	 * Scales the speed of the swerve module state based on the configuration's max
	 * speed.
	 * 
	...
	public void resetAngle(Rotation2d angle)
	{
		...
		{
			externalEncoder.get()
					.setAbsoluteOffset(MathUtil.inputModulus(
							angle.getRotations() - externalEncoder.get().getAbsolutePosition()
									+ externalEncoder.get().getAbsoluteOffset(),
							0, externalEncoder.get().getConversionFactor()));
		} else if (turnController.getSelectedEncoder() instanceof NFRAbsoluteEncoder)
		{
			...
			encoder.setAbsoluteOffset(MathUtil.inputModulus(
					angle.getRotations() - encoder.getAbsolutePosition() + encoder.getAbsoluteOffset(), 0,
					encoder.getConversionFactor()));
		}
	}

	/**
	 * Initializes the sendable data that is passed when published to network
	 * tables.
	 * 
	...
	/**
	 * Creates a traditional Mk3 swerve module with standard gearing and two falcon
	 * 500s, as well as a cancoder.
	 * 
			...
			boolean invertDrive)
	{
		NFRSwerveModuleConfiguration config = new NFRSwerveModuleConfiguration(name)
				...
				.withGearboxes(DCMotor.getFalcon500(1), DCMotor.getFalcon500(1)).withMOIs(1.2, 1.2)
		...
		driveMotor.getSelectedEncoder()
				.setConversionFactor(Mk3SwerveConstants.kWheelCircumference / Mk3SwerveConstants.kDriveGearRatioSlow);
